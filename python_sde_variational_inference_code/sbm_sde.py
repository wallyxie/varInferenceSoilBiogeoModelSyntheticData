...
def litter_drift_and_diffusion_scon(C_vector, T_span, batch_size = 3):
    '''
    Returns SCON system exogenous litter input drift. Time t is on hourly scale.
    '''
    SOC, DOC, MBC = [C_vector[:, i:i + 1, :] for i in range(batch_size)] #Get current system pool density values from C_vector tensor.
...
def drift_and_diffusion_scon(C_vector, T_span, scon_params_dict, temp_ref, batch_size = 3):
    '''
