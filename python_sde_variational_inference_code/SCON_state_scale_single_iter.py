...
CUDA_ID = 1
dt = .2
T = 2000 #Run simulation for 2000 hours.
N = int(T / dt) 
...
sigma_SOC_scale = 0.01
sigma_DOC_scale = 0.01
sigma_MBC_scale = 0.01

scon_params_dict = {'u_M': u_M, 'a_SD': a_SD, 'a_DS': a_DS, 'a_M': a_M, 'a_MSC': a_MSC, 'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref, 'Ea_S': Ea_S, 'Ea_D': Ea_D, 'Ea_M': Ea_M, 'sigma_SOC_scale': sigma_SOC_scale, 'sigma_DOC_scale': sigma_DOC_scale, 'sigma_MBC_scale': sigma_MBC_scale}
...
def analytical_steady_state_init_con(SOC_input, DOC_input, scon_params_dict = scon_params_dict):
    '''
    ...
    Expected scon_params_dict = {'u_M': u_M, 'a_SD': a_SD, 'a_DS': a_DS, 'a_M': a_M, 'a_MSC': a_MSC, 'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref, 'Ea_S': Ea_S, 'Ea_D': Ea_D, 'Ea_M': Ea_M, 'sigma_SOC_scale': sigma_SOC_scale, 'sigma_DOC_scale': sigma_DOC_scale, 'sigma_MBC_scale': sigma_MBC_scale}
    '''
...
def drift_diffusion_scon(C_vector, T_span_tensor, I_S_tensor, I_D_tensor, scon_params_dict, temp_ref):
    ...
    SOC, DOC, MBC =  torch.chunk(C_vector, state_dim, -1) #Partition SOC, DOC, and MBC values. Split based on final C_vector dim, which specifies state variables and is also dim#2. 
    current_temp = temp_gen(T_span_tensor, temp_ref) #Obtain temperature function vector across span of times.
    drift_vector = torch.empty_like(C_vector, device=C_vector.device) #Initiate tensor with same dims as C_vector to assign drift.
    ...
    #diffusion_matrix_sqrt_diag = torch.empty_like(C_vector, device=C_vector.device) #Create tensor to assign diffusion matrix elements.
    ...
    diffusion_matrix_sqrt[:, :, 0 : 1, 0] = torch.sqrt(LowerBound.apply(SOC * scon_params_dict['sigma_SOC_scale'], 1e-9)) #SOC diffusion standard deviation
    diffusion_matrix_sqrt[:, :, 1 : 2, 1] = torch.sqrt(LowerBound.apply(DOC * scon_params_dict['sigma_DOC_scale'], 1e-9)) #DOC diffusion standard deviation
    diffusion_matrix_sqrt[:, :, 2 : 3, 2] = torch.sqrt(LowerBound.apply(MBC * scon_params_dict['sigma_MBC_scale'], 1e-9)) #MBC diffusion standard deviation
    #diffusion_matrix_sqrt_diag[:, :, 0 : 1] = torch.sqrt(LowerBound.apply(SOC * scon_params_dict['sigma_SOC_scale'], 1e-9)) #SOC diffusion standard deviation
    #diffusion_matrix_sqrt_diag[:, :, 1 : 2] = torch.sqrt(LowerBound.apply(DOC * scon_params_dict['sigma_DOC_scale'], 1e-9)) #DOC diffusion standard deviation
    #diffusion_matrix_sqrt_diag[:, :, 2 : 3] = torch.sqrt(LowerBound.apply(MBC * scon_params_dict['sigma_MBC_scale'], 1e-9)) #MBC diffusion standard deviation
    #diffusion_matrix_sqrt = torch.diag_embed(diffusion_matrix_sqrt_diag)
...
obs_model = ObsModel(times = obs_times, mu = obs_means_T, scale = torch.tensor([1., 0.01, 0.1]).reshape([1, 3]))
...
train_scon(10000, 2, T_span_tensor, I_S_tensor, I_D_tensor, scon_params_dict)
