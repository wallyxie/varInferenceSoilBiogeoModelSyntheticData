...
class SDEFlow(nn.Module):
    ...
    def __init__(self, DEVICE, OBS_MODEL, STATE_DIM, T, DT, N, cond_inputs = 1, num_layers = 5):
        super().__init__()
        self.device = DEVICE
    ...
    def forward(self, BATCH_SIZE, *args, **kwargs):
        ...
        obs_tile = self.obs_model.mu[None, :, 1:, None].repeat(BATCH_SIZE, self.state_dim, 1, steps_bw_obs).reshape(BATCH_SIZE, self.state_dim, -1)
        times = torch.arange(self.dt, self.t + self.dt, self.dt, device = eps.device)[(None,) * 2].repeat(BATCH_SIZE, self.state_dim, 1).transpose(-2, -1).reshape(BATCH_SIZE, 1, -1)
        ildjs = []
