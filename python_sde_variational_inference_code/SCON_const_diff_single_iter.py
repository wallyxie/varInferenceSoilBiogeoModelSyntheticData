...
scon_params_dict = {'u_M': u_M, 'a_SD': a_SD, 'a_DS': a_DS, 'a_M': a_M, 'a_MSC': a_MSC, 'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref, 'Ea_S': Ea_S, 'Ea_D': Ea_D, 'Ea_M': Ea_M, 'sigma_SOC': sigma_SOC, 'sigma_DOC': sigma_DOC, 'sigma_MBC': sigma_MBC}
...
def analytical_steady_state_init_con(SOC_input, DOC_input, scon_params_dict = scon_params_dict):
    ...
    Expected scon_params_dict = {'u_M': u_M, 'a_SD': a_SD, 'a_DS': a_DS, 'a_M': a_M, 'a_MSC': a_MSC, 'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref, 'Ea_S': Ea_S, 'Ea_D': Ea_D, 'Ea_M': Ea_M, 'sigma_SOC': sigma_SOC, 'sigma_DOC': sigma_DOC, 'sigma_MBC': sigma_MBC}
    '''
...
def drift_diffusion_scon(C_vector, T_span_tensor, I_S_tensor, I_D_tensor, scon_params_dict, temp_ref):
    ...
    diffusion_matrix_sqrt_single = torch.diag(torch.sqrt(LowerBound.apply(torch.as_tensor([scon_params_dict['sigma_SOC'], scon_params_dict['sigma_DOC'], scon_params_dict['sigma_MBC']]), 1e-9))) #Create single diffusion matrix by diagonalizing constant noise scale parameters.
    diffusion_matrix_sqrt = diffusion_matrix_sqrt_single.expand(drift_vector.size(0), drift_vector.size(1), state_dim, state_dim) #Expand diffusion matrices across all paths and across discretized time steps.
