...
class SCON_multi_optim(SBM_SDE):
    ...
    def drift_diffusion(
        ...
        SOC, DOC, MBC = torch.chunk(c_path_drift_diffusion, self.state_dim, -1) # (batch_size, num_seqs, N-1, 1)
        ...
        if self.DIFFUSION_TYPE == 'C':
            ...
            diffusion_sqrt = torch.zeros([drift.size(0), drift.size(1), 1, self.state_dim, self.state_dim], device = drift.device) # (batch_size, num_seqs, 1, state_dim, state_dim)
            ...
            #Allocate diffusion tensor based on diffusion type.
            diffusion_sqrt = torch.zeros([drift.size(0), drift.size(1), drift.size(2), self.state_dim, self.state_dim], device = drift.device) # (batch_size, num_seqs, N-1, state_dim, state_dim)
...
class SAWB_multi_optim(SBM_SDE):
    ...
    def drift_diffusion(
        ...
        if self.DIFFUSION_TYPE == 'C':
            ...
            diffusion_sqrt = torch.zeros([drift.size(0), drift.size(1), 1, self.state_dim, self.state_dim], device = drift.device) 
