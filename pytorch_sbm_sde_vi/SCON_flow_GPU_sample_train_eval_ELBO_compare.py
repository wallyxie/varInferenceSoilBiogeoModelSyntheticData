...
net.eval()
with torch.no_grad():
    x, log_prob = net(net_eval_batch_size)
    print('x = ', x)
    theta_dict, theta, log_q_theta, parent_loc_scale_dict = q_theta(x.size(0))
    ...
    print('\nx.size() =', x.size())
    print(f'\nnet.eval() with {num_layers} layers has neg_ELBO = {neg_ELBO}')

#Compute net.train() test ELBO.
net.train()
x, log_prob = net(net_train_batch_size)
print('x = ', x)
theta_dict, theta, log_q_theta, parent_loc_scale_dict = q_theta(x.size(0))
log_p_theta = p_theta.log_prob(theta).sum(-1)
if fix_theta_dict:
...
print('\nobs_model', obs_model(x_add_CO2))
print('\nx.size() =', x.size())
print(f'\nnet.train() with {num_layers} layers has neg_ELBO = {neg_ELBO}')
