...
class SCON_fix_u_M_a_MSC_a_M(SBM_SDE):
    ...
    def drift_diffusion_add_CO2(
        ...
        Accepts states x and dictionary of parameter samples.
        Returns SCON drift and diffusion tensors corresponding to state values and parameter samples, along with tensor of states x concatenated with CO2.  
        Expected SCON_params_dict = {'a_SD': a_SD, 'a_DS': a_DS, 'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref, 'Ea_S': Ea_S, 'Ea_D': Ea_D, 'Ea_M': Ea_M, '[cs]_SOC': [cs]_SOC, '[cs]_DOC': [cs]_DOC, '[cs]_MBC': [cs]_MBC}
        '''
        c_path_drift_diffusion = C_PATH[:, :-1, :]
        i_S_tensor_drift_diffusion = self.i_S[:, 1:, :]
        i_D_tensor_drift_diffusion = self.i_D[:, 1:, :]
        ...
        #Drift is calculated.
        drift_SOC = i_S_tensor_drift_diffusion + SCON_params_dict_res['a_DS'] * k_D * DOC + 0.5 * 0.5 * k_M * MBC - k_S * SOC
        drift_DOC = i_D_tensor_drift_diffusion + SCON_params_dict_res['a_SD'] * k_S * SOC + 0.5 * 0.5 * k_M * MBC - (0.0016 + k_D) * DOC
        drift_MBC = 0.0016 * DOC - k_M * MBC
        ...
        #Compute CO2.
        CO2 = (k_S_full * SOC_full * (1 - SCON_params_dict_res['a_SD'])) + (k_D_full * DOC_full * (1 - SCON_params_dict_res['a_DS'])) + (k_M_full * MBC_full * 0.5)
