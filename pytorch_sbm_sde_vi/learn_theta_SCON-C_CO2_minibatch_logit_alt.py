...
#Load parameterization of priors (of theta)
...
#Initial condition prior means (of x)
x0_SCON = [65, 0.4, 2.5]
...
csv_data_path = os.path.join('generated_data/', 'SCON-C_CO2_logit_alt_sample_y_t_1000_dt_0-01_sd_scale_0-333.csv')
...
net, q_theta, p_theta, obs_model, ELBO_hist, list_parent_loc_scale, SBM_SDE_instance = train2(
        ...
        theta_dist, MINIBATCH_SIZE = minibatch_size,
        BYPASS_NAN = False, LR_DECAY = 0.9, DECAY_STEP_SIZE = 25000, PRINT_EVERY = 50)

#Save net and ELBO files.
now = datetime.now()
now_string = 'SCON-C_CO2_logit_alt' + now.strftime('_%Y_%m_%d_%H_%M_%S')
...
true_theta = torch.load('generated_data/SCON-C_CO2_logit_alt_sample_y_t_1000_dt_0-01_sd_scale_0-333_rsample.pt', map_location = active_device)
