...
def drift_diffusion_SCONR_C_fix_u_M_a_Ea_c(C_PATH, T_SPAN_TENSOR, I_S_TENSOR, I_D_TENSOR, TEMP_TENSOR, TEMP_REF, SCONR_C_fix_u_M_a_Ea_c_params_dict):
    '''
    Returns SCONR fix u_M, a, and Ea "constant diffusion parameterization" drift vectors and diffusion matrices.
    Expected SCONR_C_fix_u_M_a_Ea_c_params_dict = {'k_S_ref': k_S_ref, 'k_D_ref': k_D_ref, 'k_M_ref': k_M_ref}
    '''
    ...
    k_S = arrhenius_temp_dep(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_S_ref'], TEMP_TENSOR, 55, TEMP_REF) #Apply vectorized temperature-dependent transformation to k_S_ref.
    k_S = k_S.permute(2, 1, 0) #Get k_S into appropriate dimensions. 
    k_D = arrhenius_temp_dep(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_D_ref'], TEMP_TENSOR, 48, TEMP_REF) #Apply vectorized temperature-dependent transformation to k_D_ref.
    k_D = k_D.permute(2, 1, 0) #Get k_D into appropriate dimensions.
    k_M = arrhenius_temp_dep(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_M_ref'], TEMP_TENSOR, 48, TEMP_REF) #Apply vectorized temperature-dependent transformation to k_M_ref.
    k_M = k_M.permute(2, 1, 0) #Get k_M into appropriate dimensions.
    ##Repeat and permute parameter values to match dimension sizes
    #SCONR_C_fix_u_M_a_Ea_c_params_dict_rep = dict((k, v.repeat(1, T_SPAN_TENSOR.size(1), 1).permute(2, 1, 0)) for k, v in SCONR_C_fix_u_M_a_Ea_c_params_dict.items()) #Not necessary for this experiment, as all non-temperature-influenced parameters have been fixed.   
    #Drift is calculated.
    ...
    #CO2 = (k_S * SOC * (1 - SCONR_C_fix_u_M_a_Ea_c_params_dict_rep['a_SD'])) + (k_D * DOC * (1 - SCONR_C_fix_u_M_a_Ea_c_params_dict_rep['a_DS'])) + (k_M * MBC * (1 - SCONR_C_fix_u_M_a_Ea_c_params_dict_rep['a_M'])) 
    ...
    diffusion_sqrt_single = torch.diag_embed(torch.sqrt(torch.stack([torch.Tensor([0.1]).repeat(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_S_ref'].size(0)), torch.Tensor([0.002]).repeat(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_D_ref'].size(0)), torch.Tensor([0.002]).repeat(SCONR_C_fix_u_M_a_Ea_c_params_dict['k_M_ref'].size(0))], 1))) #Create single diffusion matrix by diagonalizing constant noise scale parameters. 
...
#SCONR-C fix u_M, a, and Ea experiment
def drift_diffusion_SCONR_C_fix_u_M_a_Ea(C_PATH, T_SPAN_TENSOR, I_S_TENSOR, I_D_TENSOR, TEMP_TENSOR, TEMP_REF, SCONR_C_fix_u_M_a_Ea_params_dict):
    '''
    Returns SCONR fix u_M, a, and Ea "constant diffusion parameterization" drift vectors and diffusion matrices.
