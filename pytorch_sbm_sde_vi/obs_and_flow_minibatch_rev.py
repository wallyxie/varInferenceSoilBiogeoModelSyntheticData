...
class SDEFlowMinibatch(nn.Module):
    ...
    def __init__(self, DEVICE, OBS_MODEL_MINIBATCH, STATE_DIM, T, N, THETA_DIM,
                 ...
                 POSITIVE = True, THETA_COND = 'convolution', REVERSE = False):
        super().__init__()
        ...
        for i in range(self.num_layers):
            ...
            layers += [PermutationLayer(self.state_dim, REVERSE=REVERSE)]
    ...
    def forward(self, bsz, lidx, ridx, *args, **kwargs):
        ...
        assert eps.shape == log_prob.shape # (batch_size, 1, n * state_dim)
...
class ObsModelMinibatch(ObsModel):
    ...
    def forward(self, x, theta, lidx=0, ridx=None):
        ...
        #print(active_lidx, self.idx[obs_lidx:obs_ridx])
