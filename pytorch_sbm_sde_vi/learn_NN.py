...
t_span_tensor = torch.reshape(torch.Tensor(t_span), [1, n, 1]).to(gpu) #T_span needs to be converted to tensor object. Additionally, facilitates conversion of I_S and I_D to tensor objects.
...
temp_tensor = temp_gen(t_span_tensor, temp_ref, temp_rise).to(gpu)

#Obtain SOC and DOC pool litter input vectors for use in flow SDE functions.
i_s_tensor = i_s(t_span_tensor).to(gpu) #Exogenous SOC input function
i_d_tensor = i_d(t_span_tensor).to(gpu) #Exogenous DOC input function

#Call training loop function for SCON-C.
net, ELBO_hist = train(gpu, pretrain_lr, train_lr, niter, piter, batch_size, num_layers,
