...
t = 1100 #5000. Reduced to see impact on memory. #In hours.
n = int(t / dt_flow) + 1
...
#Training parameters
niter = 3001 #Total number of training iterations, including ELBO iterations.
piter = 120 #Number of pre-training iterations.
pretrain_lr = 8e-3 #Norm regularization learning rate
train_lr = 2e-4 #ELBO learning rate
...
net, ELBO_hist = train(active_device, pretrain_lr, train_lr, niter, piter, batch_size, num_layers,
          ...
          LEARN_THETA = False, LR_DECAY = 0.95, DECAY_STEP_SIZE = 2800, PRINT_EVERY = 10)
