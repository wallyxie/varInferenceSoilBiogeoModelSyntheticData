...
priors_file = 'generated_data/SCONR_C_fix_a_Ea_logit_alt_sample_y_t_1000_dt_0-01_sd_scale_0-333_hyperparams.pt'
SCONR_C_fix_a_Ea_priors_details = {k: v.to(active_device) for k, v in torch.load(priors_file).items()}
...
csv_data_path = os.path.join('generated_data/', 'SCONR_C_fix_a_Ea_logit_alt_sample_y_t_1000_dt_0-01_sd_scale_0-333.csv')
...
net, q_theta, p_theta, obs_model, ELBO_hist, list_parent_loc_scale = train(
        ...
        t_span_tensor, i_s_tensor, i_d_tensor, temp_tensor, temp_ref,
        drift_diffusion_SCONR_C_fix_a_Ea, x0_prior_SCON, SCONR_C_fix_a_Ea_priors_details, theta_dist,
        LEARN_THETA = True, LR_DECAY = 0.8, DECAY_STEP_SIZE = 50000, PRINT_EVERY = 100)

#Save net and ELBO files.
now = datetime.now()
now_string = 'SCONR_C_fix_a_Ea_logit_alt_' + now.strftime('%Y_%m_%d_%H_%M_%S')
...
plot_states_post(x, obs_model, state_dim_SCON, niter, piter, t, dt_flow, batch_size, eval_batch_size, num_layers, train_lr, prior_scale_factor, plots_folder, now_string, ymin_list = [0, 0, 0], ymax_list = [100., 10., 10.])
