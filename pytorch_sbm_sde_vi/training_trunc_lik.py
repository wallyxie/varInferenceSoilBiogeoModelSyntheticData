...
def calc_log_lik1(C_PATH: torch.Tensor,
    ...
    elif TRANS_DIST == 'TruncatedNormal':
...
def calc_log_lik2(C_PATH: torch.Tensor,
    ...
    if LEARN_CO2:
        if TRANS_DIST == 'Normal':
            ...
            drift, diffusion_sqrt, x_add_CO2 = SBM_SDE_CLASS.drift_diffusion_add_CO2(C_PATH, PARAMS_DICT, diffusion_matrix=False)
            euler_maruyama_state_sample_object = TruncatedNormal(loc = C_PATH[:, :-1, :] + drift * DT, scale = diffusion_sqrt * math.sqrt(DT), a = 0, b = float('inf'))
...
def train1(DEVICE, ELBO_LR, NITER, BATCH_SIZE, NUM_LAYERS,
        ...
        THETA_DIST = None, THETA_POST_DIST = None, THETA_POST_INIT = None, TRANS_DIST = None,
    ...
    with tqdm(total = NITER, desc = f'Learning SDE and hidden parameters.', position = -1) as tq:
        for it in range(NITER):
            ...
            if LEARN_CO2:
                x_add_CO2 = SBM_SDE.add_CO2(C_PATH, theta_dict)
