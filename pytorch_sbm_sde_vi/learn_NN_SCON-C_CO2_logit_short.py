...
elbo_warmup_iter = 50 # 2000
elbo_warmup_lr = 1e-6
ptrain_iter = 0
ptrain_alg = 'L1'
batch_size = 31 #38
eval_batch_size = 31 #38
obs_error_scale = 0.1
...
params_dict = torch.load(os.path.join('generated_data/', 'SCON-C_CO2_logit_short_2022_01_20_08_53_sample_y_t_5000_dt_0-01_sd_scale_0-25_rsample.pt'), map_location = active_device)
...
with torch.no_grad():
    for i in range(batch_multiples):
        ...
        if learn_CO2:
            ...
            _x = SBM_SDE_instance.add_CO2(_x, params_dict) #Add CO2 to x tensor if CO2 is being fit.
        if i == 0:
            x_eval = _x
        else:
