...
with torch.no_grad():
    for i in range(batch_multiples):
        ...
        if learn_CO2:
            ...
            theta_dict_tensor = {k: torch.tensor(v).to(device) for k, v in params_dict.items()}
            _x = SBM_SDE_instance.add_CO2(_x, theta_dict_tensor) #Add CO2 to x tensor if CO2 is being fit.
        if i == 0:
            x_eval = _x
        else:
