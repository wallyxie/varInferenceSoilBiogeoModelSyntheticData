...
def plot_elbo(elbo_hist, niter, warmup_iter, t, dt, batch_size, eval_batch_size, num_layers, train_lr, warmup_lr, sd_scale, plots_folder, now_string, xmin = 0, ymax = None, yscale = 'linear'):
    ...
    plt.savefig(os.path.join(plots_folder, f'ELBO_iter_{niter}_warmup_{warmup_iter}_t_{t}_dt_{dt}_batch_{batch_size}_samples_{eval_batch_size}_layers_{num_layers}_lr_{train_lr}_warmup_lr_{warmup_lr}_sd_scale_{sd_scale}_{now_string}.png'), dpi = 300)
    
def plot_states_post(x, q_theta, obs_model, SBM_SDE_CLASS, niter, warmup_iter, t, dt, batch_size, eval_batch_size, num_layers, train_lr, warmup_lr, sd_scale, plots_folder, now_string, FIX_THETA_DICT = None, LEARN_CO2 = False, ymin_list = None, ymax_list = None):
    ...
    fig.savefig(os.path.join(plots_folder, f'net_iter_{niter}_warmup_{warmup_iter}_t_{t}_dt_{dt}_batch_{batch_size}_samples_{eval_batch_size}_layers_{num_layers}_lr_{train_lr}_warmup_lr_{warmup_lr}_sd_scale_{sd_scale}_{now_string}.png'), dpi = 300)

def plot_states_NN(x, params_dict, obs_model, SBM_SDE_CLASS, niter, warmup_iter, t, dt, batch_size, eval_batch_size, num_layers, train_lr, warmup_lr, sd_scale, plots_folder, now_string, LEARN_CO2 = False, ymin_list = None, ymax_list = None):
...
def plot_theta(p_theta, q_theta, true_theta, niter, warmup_iter, t, dt, batch_size, eval_batch_size, num_layers, train_lr, warmup_lr, sd_scale, plots_folder, now_string, ncols=4):
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(plots_folder, f'theta_iter_{niter}_warmup_{warmup_iter}_t_{t}_dt_{dt}_batch_{batch_size}_samples_{eval_batch_size}_layers_{num_layers}_lr_{train_lr}_warmup_lr_{warmup_lr}_sd_scale_{sd_scale}_{now_string}.png'), dpi = 300)
