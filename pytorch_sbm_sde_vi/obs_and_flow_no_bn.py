...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, inp_cha, out_cha, stride = 1, first = True):
        super().__init__()
...
class ResNetBlockUnMasked(nn.Module):
    ...
    def __init__(self, inp_cha, out_cha, stride = 1):
        super().__init__()
...
class AffineLayer(nn.Module):
    ...
    def forward(self, x, COND_INPUTS): # x.shape == (batch_size, 1, n * state_dim)
        ...
        #Re-zero trick execution.
        mu, sigma = self.alpha * mu, LowerBound.apply(self.beta * sigma + self.gamma, 1e-7)
